...
│       ├── external-directory.ts describeExternalDirectoryGate - pure descriptor/bypass factory; builds an `AccessPath` and uses `accessPath.matchValues()` for `resolver.resolvePathPolicy(..., "external_directory")` and `accessPath.boundaryValue()` for the outside-CWD boundary and infra-read checks (#418, #476)
