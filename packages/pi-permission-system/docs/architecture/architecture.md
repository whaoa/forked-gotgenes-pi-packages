...
| Indirection-wrapper coverage                                         | inline-shell wrappers only ([#481]) | prefix wrappers re-targeted, `xargs`/`find -exec` floored |
| Flat `src/` root modules                                             | 62                                  | ≤ 59 (`src/path/` seeded)                                 |
...
#### Step 5: Re-target or floor indirection wrappers ([#490])
...
Direction confirmed 2026-07-10: re-target visible prefix wrappers (`sudo`, `env`, `time`, `nohup`, `timeout`, `nice`) at the inner command; floor `xargs` / `find -exec` to `ask` like the opaque wrappers.

- **Smell:** Category C (boundary flaw in bash command enumeration).
- **Target:** `access-intent/bash/command-enumeration.ts` (+ tests).
- **Outcome:** a `<cmd> *` deny/ask rule governs `sudo <cmd> …`; `xargs` / `find -exec` cannot ride a permissive allow (test-gated).
