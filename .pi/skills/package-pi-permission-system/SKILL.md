...
- On win32 a backslash is a path separator, so a backslash-relative bash argument (`cat dir\file`) is gated by the `path` surface the same as `dir/file` ([#520]); the broad rule-candidate classifier recognizes it under the `windowsSeparators` option (from `PathNormalizer.usesWindowsSeparators()`), and it resolves through the ordinary win32 `forBashToken` (`plain`) branch.
